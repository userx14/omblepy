...
async def syncTimeWithSystemTime(btobj):
    ...
    _, _, month, year, hour, day, second, minute = [int(byte) for byte in readTimeSyncDataByteArray]
    print(f"device is set to date: {datetime.datetime(year + 2000, month, day, hour, minute, second).strftime('%Y-%m-%d %H:%M:%S')}")

    #set current system time
    currentTime = datetime.datetime.now()
    setNewTimeDataBytes = settingsDataByteArray[timeSyncOffset: timeSyncOffset + 2] #just take the first two bytes from eeprom
